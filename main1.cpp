...
int main(int argc, char **argv)
{
	...
	for(int i=1; i<argc; ++i)
	{
		...
		else if(argv[i][0]=='-' && argv[i][1]=='w')
		{
			if(continue_file)
			{
				cout << "Sites-downloader can use only one \"wrongs\" file!\n";
				abort();
			}
			continue_file=true;
			string _s;
	...
	while(!sites.empty())
	{
		...
		if(file.good())
		{
			...
			if(file.good() && out.good() && ___empty==0)
			{
				...
				if(compare_with_end(k,".js") || compare_with_end(k,".png") || compare_with_end(k,".jpg") || compare_with_end(k,".gif") || compare_with_end(k,".ico"))
					while(file.good())
					{
						getline(file,input);
						/*aho::find(input);
